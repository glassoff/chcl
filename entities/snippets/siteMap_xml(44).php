...
# ---------------------------------------------
$docs = getDocs($modx,$startid,$priority,$changefreq,$excludeTV);//print_r($docs);die();
...
##
$remainingTemplates[999] = 'Item template';
...
///print_r($remainingTemplates);die();
$output= array();
...
switch ($format)
{
	...
	<url>
		<loc>[(site_url)][~1309~]</loc>
		<priority>0.8</priority>
	</url> 	    
    ';
    
    foreach ($docs as $doc) {//print_r($doc);die();
        $url = '[(site_url)][~'.$doc['id'].'~]';
        ##
        if($doc['type']=='item')
        	$url = '[(site_url)]catalog/item?id='.$doc['id'];
        $date = $doc['editedon'];
...
##
function getItems($parent, $priority,$changefreq,$excludeTV){
	global $modx;
	$items = array();
	$sql = "SELECT * FROM modx_site_ec_items WHERE parent='$parent' AND published='1' AND deleted='0'";
