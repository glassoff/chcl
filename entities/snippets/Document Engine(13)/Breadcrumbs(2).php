...
// Configuration Settings
    (isset($showCurrentCrumb)) ? intval($showCurrentCrumb) : $showCurrentCrumb = 0;
    // $maxCrumbs [number]       
    ...
    // Just in case you want to have a home link,
    // but want to call it something else
    (isset($startPageId)) ? $startPageId : $startPageId = $modx->config['site_start'];
    $currPageId = (isset($currPageId)) ? $currPageId :  $modx->documentIdentifier; 
    
    $showStartCrumb = (isset($showStartCrumb)) ? $showStartCrumb :  0; 
...
// ***********************************
// Check for home page
if ($showCrumbsAtHome || (!$showCrumbsAtHome && ($currPageId != $modx->config['site_start'])) ){

    //initialize crumb array
    $ptarr = array();
    // get current page parent info
    $docInfo = $modx->getDocument($currPageId,'id,pagetitle,menutitle,published,deleted,parent,hidemenu,description,longtitle,introtext');
    $pid = $docInfo['parent'];
    // show current page, as link or not
    if ($showCurrentCrumb == 1){
        ...
        if ($currentAsLink && (!$respectHidemenu || ($respectHidemenu && $docInfo['hidemenu'] != 1 ))){
            ...
            $ptarr[] = '<a class="B_currentCrumb" href="[~'.$currPageId.'~]" title="'.$descriptionToUse.'">'.$titleToShow.'</a>';
        } else {
    ...
    while (($parent=$modx->getPageInfo($pid,!$pathThruUnPub,"id,pagetitle,menutitle,published,deleted,parent,hidemenu,description,longtitle,introtext")) && ($crumbCount < $maxCrumbs)) {
        if ($parent['published'] && !$parent['deleted'] && $parent['id'] != $modx->config['site_start']){
            if (!$respectHidemenu || ($respectHidemenu && $parent['hidemenu'] != 1 )){
                ...
                if ($showStartCrumb == '0' && $startPageId == $parent['id']) {
                } else $ptarr[] = '<a class="B_crumb" href="[~'.$parent['id'].'~]" title="'.$descriptionToUse.'">'.$titleToShow.'</a>';
            }
        } // end if   
        $pid = $parent['parent'];
        $crumbCount++;
    ...
    if ($showHomeCrumb && $showHomeCrumb && ($currPageId != $modx->config['site_start'])){
        $ptarr[] = '<a class="B_homeCrumb" href="[~'.$startPageId.'~]" title="'.$homeCrumbDescription.'">'.$homeCrumbTitle.'</a>';
    }
    if (sizeof($ptarr)< 2) return;
