...
if($_POST['send']){
	...
	//##
	$content = fgets($pointer, $_FILES['usersFile']['size']);
	$parts = explode(',', $content);
	foreach($parts as $part){
		preg_match('#(.*?)\s*<(.+)>#is', $part, $match);
		$companyType = getCompanyType($match[1]);	
		$companyName = getCompanyName($match[1], $companyType);
		$data = array(
			'email' => getEmail($match[2]),
			'type' => $companyType,
			'company' => $companyName,
			'sname' => $companyName
		);	
...
function getCompanyName($str, $type){
	$name = trim( str_replace($type, '', $str) );
