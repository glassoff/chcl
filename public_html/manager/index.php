...
// get the eC settings from the database
include_once "ec_settings.inc.php";
...
$time = time();
$sql =	"DELETE FROM ".$modx->getFullTableName('active_managers')." WHERE act_ses_timeout < $time";
mysql_query($sql);
...
/********************************************************************/
	...
	case "4000" :             
        include_once "ecommerce/actions/cities.list.php";        
	break;

	case "4999" :
        // get the list page of a category items of eCommerce
        include_once "header.inc.php";
        include_once "ecommerce/actions/overview_ec.static.php";
        include_once "footer.inc.php";
	break;
	case "5000" :
        // preview
        if($_GET['action']!='1c_tovars')include_once "header.inc.php";
        include_once "ecommerce/actions/list_ec.items.dynamic.php";
        include_once "footer.inc.php";
	break;
	...
	case "5001" :
        // new item
        include_once "header.inc.php";
        include_once "ecommerce/actions/mutate_ec.item.dynamic.php";       
        include_once "footer.inc.php";
	break;
	...
	case "5003" :
        // save item       
        include_once "ecommerce/processors/save_ec_item.processor.php";        
	break;
	
	  case "5088" :
        // get the edit web user page
        include_once "header.inc.php";
        include_once "ecommerce/actions/user_uved.php";
        include_once "footer.inc.php";
    break;
  case "5089" :
        // get the edit web user page
        include_once "header.inc.php";
        include_once "ecommerce/actions/user_uved2.php";
        include_once "footer.inc.php";
    break;
	...
	case "5004" :
		// Item Preview
        include_once "header.inc.php";
        include_once "ecommerce/actions/item_data.static.php";
        include_once "footer.inc.php";      
	break;
	...
	case "5005" :
        // Item list Wait page   
        include_once "header.inc.php";
        include_once "ecommerce/actions/wait.static.php";  
        include_once "footer.inc.php";      
	break;

	case "5006" :
        include_once "ecommerce/processors/metatags.processor.php";
	break;

	case "5007" :
        // Delete item     
        include_once "ecommerce/processors/delete_ec_item.processor.php";       
       
	break;

	case "5008" :
        // Undelete item     
        include_once "ecommerce/processors/undelete_ec_item.processor.php";       
       
	break;

	case "5009" :
        // Remove item     
        include_once "ecommerce/processors/remove_ec_item.processor.php";       
       
	break;

	case "5010" :
        // Publish    
        include_once "ecommerce/processors/publish_ec_item.processor.php";    
	break;
	...
	case "5011":
		// Unpaublish
		include_once "ecommerce/processors/unpublish_ec_item.processor.php";
	break;
	...
	case "5015":
		// Group delete
		include_once "ecommerce/processors/group_ec_item.processor.php";
	break;	
	...
	case "900":
		// AjaxProvider
		include_once "processors/ajaxprovider.php";
	break;
	...
	case "5022":		
		// ajax handler
		include_once "ecommerce/ajaxprocessors/ajax.php";		
	break;
	
	case "5021":
		include_once "header.inc.php";
		include_once "ecommerce/actions/tax_maintenance.dynamic.php";
		include_once "footer.inc.php";
	break;
	
	case "5023":
		include_once "header.inc.php";
		include_once "ecommerce/actions/discount_maintenance.dynamic.php";
		include_once "footer.inc.php";
	break;
	...
	case "10009" :
        $fcontents = file ('./index.csv'); 
        //$sql = "DELETE FROM ".$modx->getFullTableName("site_ec_regions");
        //mysql_query($sql);
        //$sql = "DELETE FROM ".$modx->getFullTableName("site_ec_cities");
        //mysql_query($sql);        
        $cnt = 0;  
        $cnt1 = 0; 
		...
		for($i=1; $i<sizeof($fcontents); $i++) { 
		     ...
		     $arr = explode(";", $line);  
			 if (!empty($arr[5])) { 	
			     $sql = "insert into ".$modx->getFullTableName("site_ec_regions")."(postcode,name) values ('".$arr[0]."','".$arr[5]."');";
			     @mysql_query($sql);
			     echo "<b>$i-$rcode</b>".$sql ."<br>\n";
				 if(mysql_error()) {
				       echo mysql_error() ."<br>\n";
				 } 	 
		     }
		     //$sql = "insert into $dbase.`klard`(name,sokr,code,postindex,gninmb,uno,ocatd,status) values ('".implode("','", $arr) ."')";     
		}
        echo "<b>Kol-$cnt;</b>";
    break;
	
	case "10000" :
		//echo $name = ucwords(strtolower('АВТОНОМНЫЙ ОКРУГ'));
		//exit;
        $fcontents = file ('./index.csv'); 
        $sql = "DELETE FROM ".$modx->getFullTableName("site_ec_regions");
        mysql_query($sql);
        ...
        $sokrs = array('Республика','Автономный Округ','Край','Область','Автономная Область');  
        $sokrs_ = array('Респ','АО','край','обл','Аобл');   
        set_time_limit(1000000);
		# expects the csv file to be in the same dir as this script		
		for($i=1; $i<sizeof($fcontents); $i++) { 
		     $line = trim($fcontents[$i]);		     
		     $arr = explode(";", $line);  
		     if (!empty($arr[4])) $name = $arr[4];  
		     elseif (!empty($arr[5])) $name = $arr[5];
		  	 $name = str_replace($sokrs,$sokrs_,$name);
			 $sql = "insert into ".$modx->getFullTableName("site_ec_regions")."(postcode,name) values ('".$arr[0]."','".$name."');";
			 @mysql_query($sql);
			 echo "<b>$i-$rcode</b>".$sql ."<br>\n";
			 if(mysql_error()) {
			      echo mysql_error() ."<br>\n";
    ...
    case "10005" :
    	set_time_limit(1000000);       
        $sql = "SELECT * FROM ".$modx->getFullTableName("site_ec_regions");
        $rs = mysql_query($sql);        
        for ($j=0;$j<mysql_num_rows($rs);$j++) {
        	$region = mysql_fetch_assoc($rs); 
        	$name = ucwords(strtolower($region['name']));
        	$sql = "UPDATE ".$modx->getFullTableName("site_ec_regions")."SET name='$name' WHERE id=$region[id]"; 
		    mysql_query($sql);
		    echo "<b>".$sql ."<br>\n";
        }
     break;
    ...
    case "10003" :
        ...
        //for ($i = 0;$i<2;$i++) {	
			...
			for($i=1; $i<sizeof($fcontents); $i++) { 
			     $line = trim($fcontents[$i]);		     
			     $arr = explode(";", $line);
			     $name = (!empty($arr[6]) ? $arr[6]." - " :"").$arr[7];
			     $name = str_replace('Район','р-н',$name);
			     $reg = !empty($arr[4]) ? $arr[4] : $arr[5];
			     $reg = str_replace($sokrs,$sokrs_,$reg);	    
			     if (!in_array($name, $trash) && $reg == $region['name'] && !empty($arr[7])) {  
			     	$trash[] = $name;			     	
			     	$sql = "insert into ".$modx->getFullTableName("site_ec_cities")."(rid, postcode, name)";
					$sql.= "values ($region[id],'".$arr[0]."','".$name."');";
					@mysql_query($sql);
					echo "<b>$i-$rcode</b>".$sql ."<br>\n";
					if(mysql_error()) {
					    echo mysql_error() ."<br>\n";
    ...
    case "10002" :
		...
		for($i=0; $i<sizeof($fcontents); $i++) { 
		     $line = trim($fcontents[$i]);		     
		     $arr = explode(";", $line);		         
		     $sql = "UPDATE ".$modx->getFullTableName("site_ec_cities")."SET rate_zone=$arr[1] WHERE postcode='$arr[0]'"; 
		     mysql_query($sql);
		     echo "<b>$i-$rcode</b>".$sql ."<br>\n";
			 if(mysql_error()) {
			     echo mysql_error() ."<br>\n";
	...
	case "5100":
		// Ec Settings
		include_once "header.inc.php";
		include_once "ecommerce/1cpricelist/1cpricelist.dynamic.php";
		include_once "footer.inc.php";
	break;
	...
	case "5200":
		// Ec Settings
		include_once "header.inc.php";
		include_once "ecommerce/actions/mutate_ec_settings.dynamic.php";
		include_once "footer.inc.php";
	break;
	...
	case "5201":
		// Not soon
		include_once "ecommerce/processors/save_ec_settings.processor.php";
	break;
	
	case "5222":
		
		include_once "ecommerce/processors/save_ec_nobuy_noconfirm.php";
	
	break;
	
		case "5333":
		
		include_once "ecommerce/processors/send_pozdr.php";
	...
	break;
		...
		include_once "ecommerce/processors/sending.php";
	...
	case "5202":
		// Ec Settings
		include_once "header.inc.php";
		include_once "ecommerce/actions/mutate.payment_methods.php";
		include_once "footer.inc.php";
	break;
	...
	case "5203":
		// Ec Settings
		include_once "header.inc.php";
		include_once "ecommerce/actions/mutate_ec.pm.dynamic.php";
		include_once "footer.inc.php";
	break;
	...
	case "5205":
		// Ec Settings		
		include_once "ecommerce/processors/save_ec_pm.processor.php";		
	break;
	
	case "5206":
		// Not soon
		include_once "ecommerce/processors/remove_ec_pm.processor.php";
	break;
	...
	case "5301":
		// Add/Edit region processor		
		include_once "ecommerce/processors/save_ec_region.processor.php";		
	break;
	
	case "5302":
		// Add/Edit city processor		
		include_once "ecommerce/processors/save_ec_city.processor.php";		
	break;
	
	case "5303":
		// Add/Edit city processor		
		include_once "ecommerce/processors/save_ec_zones.processor.php";		
	break;
	// discounts 
	case "5400":
		// Discount list
	...
	case "5401":
		// Add discount
		include_once "header.inc.php";
		include_once "ecommerce/actions/mutate_ec.discount.dynamic.php";
		include_once "footer.inc.php";
	break;
	...
	case "5403":
		// Add/Edit discount processor		
		include_once "ecommerce/processors/save_ec_discount.processor.php";		
	break;
	
	case "5404":
		// Remove discount
		include_once "ecommerce/processors/remove_ec_discount.processor.php";
	break;
	...
	case "5500":
		...
		if (isset($_REQUEST['importorders']) && intval($_REQUEST['importorders']) == 1) {
			include_once "ecommerce/actions/list_ec.orders.dynamic.php";
			include_once "footer.inc.php";
		} else {
			include_once "header.inc.php";
			include_once "ecommerce/actions/list_ec.orders.dynamic.php";
	...
	case "5501":
		// Save changed
		include_once "header.inc.php";
		include_once "ecommerce/actions/order_details.static.php";
		include_once "footer.inc.php";
	break;
	
	case "5502":
		// Save changed
		include_once "ecommerce/processors/remove_ec_order.processor.php";
	break;
	
	case "5503":
		// Save changed
		include_once "ecommerce/processors/save_ec_order_status.processor.php";
	break;
	
	case "5555":
		// Save changed
		include_once "ecommerce/processors/save_ec_order_details.processor.php";
	break;
	
	case "5510":
		// Save changed
	
		include_once "ecommerce/processors/save_ec_order_status_change_processor.php";
	break;
	...
	case "5520":
		// Ec order list
		include_once "header.inc.php";
		include_once "ecommerce/actions/report_ec.static.php";
		include_once "footer.inc.php";
	...
	case "5521":
		// Save changed
		include_once "ecommerce/processors/report_ec.processor.php";
	break;
	...
	case "3000":
		// Tax list
		include_once "header.inc.php";
		include_once "ecommerce/actions/brand_maintenance.dynamic.php";
		include_once "footer.inc.php";
	break;
	
	case "3001":
		// Add/Edit region processor		
		include_once "ecommerce/processors/save_ec_brand.processor.php";		
	break;
	
	case "3002":
		// Tax list
		include_once "header.inc.php";
		include_once "ecommerce/actions/pack_maintenance.dynamic.php";
		include_once "footer.inc.php";
	break;
	
	case "3003":
		// Add/Edit region processor		
		include_once "ecommerce/processors/save_ec_pack.processor.php";		
	break;
...
/********************************************************************/
    case "85" :
        // get the mutate page for adding a item folder
        include_once "header.inc.php";
