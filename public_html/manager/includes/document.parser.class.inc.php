...
function money($money) {
	global $_lang; 
	return number_format($money,2,'.','')."&nbsp;".$_lang['currency'];	
}
...
function quantity($quantity) {
	global $_lang; 
	return number_format($quantity,0)."&nbsp;".$_lang['quantity'];	
}

function quantity1($quantity, $item) {//print_r($item);die();
	global $_lang; 
	if($item['package_items'] > 0 && ($item['type']=='opt' || $item['type']=='package'))
		return number_format($quantity,0)."&nbsp; упак.";
	else
		return number_format($quantity,0)."&nbsp; шт.";	
}

function datetime($datetime,$format= "%d.%m.%Y %H:%M:%S") {
	return @strftime($format,$datetime);	
}
function datetime1($datetime,$format= "%d.%m.%Y") {
	return @strftime($format,$datetime);	
}
##
function getPrices($item_id, $type = "retail"){
	global $modx;
	/*if(!$type){
	...
	$sql = "SELECT size, price FROM ".$modx->getFullTableName("site_ec_prices")." WHERE (item_id='$item_id' AND type='$type') ORDER BY size;";
	//die($sql);
	...
	if (mysql_num_rows($result)>0){
		...
		while($row = mysql_fetch_array($result)){
			...
			if ($row['price'] && $row['size']){
				$return[ ($row['size']) ] = money1($row['price']); 
			}	
		}
		return $return;
	}
...
function setPrice($id, $price_type, $size_item, $price_value){
	global $modx;
	...
	$sql = "SELECT size, retail_size FROM ".$modx->getFullTableName('site_ec_items')." WHERE (id='$id') ";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	$sizes_str = $price_type=='retail' ? $row['retail_size'] : $row['size'];
	$sizes = array_map('trim', explode(",", $sizes_str));	
	
	if(!in_array($size_item, $sizes))
		return false;
		
	$sql = "SELECT * FROM ".$modx->getFullTableName("site_ec_prices")." WHERE (item_id='$id' AND type='$price_type' AND size='$size_item');";
	$result = mysql_query($sql);
	if(mysql_num_rows($result) > 0){
		$sql = "UPDATE ".$modx->getFullTableName("site_ec_prices")." SET price='$price_value' WHERE (item_id='$id' AND type='$price_type' AND size='$size_item')";
	}
	else{
		$sql = "REPLACE INTO ".$modx->getFullTableName("site_ec_prices")." (item_id, type, size, price) VALUES ('$id', '$price_type', '$size_item', '$price_value')";
	}	
	$result = mysql_query($sql);			
}
...
class Colors{
	...
	public function __construct($itemid, $colorData = array()){
		$this->itemid = $itemid;
		
		$this->colors = (array)$colorData;
		if(!$colorData){
			$this->load();
		}
	...
	protected function load(){
		global $modx;
		$sql = "SELECT * FROM ".$modx->getFullTableName('site_ec_colors')." WHERE (item_id = '".$this->itemid."')";
	...
	protected function setStr(){
		foreach($this->colors as $color){
			$this->strColors .= $color['name'] . ', ';
		}
		if(strlen($this->strColors) > 2)
			$this->strColors = substr($this->strColors, 0, -2);
	}
...
class DocumentParser {
    ...
    function loadExtension($extname) {
        ...
        switch ($extname) {
            ...
            case 'cache_lite' :
                if (!include_once MODX_BASE_PATH . 'manager/includes/extenders/cache_lite/Lite.php')
                    return false;
...
//                header('HTTP/1.0 404 Not Found');
    ...
    function evalSnippet($snippet, $params) {
        ...
        //$cache = false;
		$snippet_id =  isset($params['id']) ? $params['id'] : 'undefined1';
		$cache_scale =  isset($params['cacheScale']) ? $params['cacheScale'] : 'global1';
		$cache_dependances =  isset($params['cacheDependances']) ? $params['cacheDependances'] : '';
	
		//$cache = false;
		if ($cache) {		    
		  
		    $cache_group = $snippet_id;
		    $dep_string = '';
		    
		    if (!empty($cache_dependances)) {
			    $cache_dependances_arr = explode(',',$cache_dependances);			    
			    foreach ($cache_dependances_arr as $k => $v) {
			    	$v_arra =  explode(':',$v); 
			    	$vtype = strtolower($v_arra[0]);
			    	$vname = $v_arra[1];
			    	switch ($vtype) {
		        		case 'session':{ 
		        			if (isset($_SESSION[$vname]) && !empty($_SESSION[$vname])) 
		        			$dep_string.=$vname.':'.$_SESSION[$vname].'#'; 	
		        		};break;
		        		case 'server':{ 
		        			if (isset($_SERVER[$vname]) && !empty($_SERVER[$vname])) 
		        			$dep_string.=$vname.':'.$_SERVER[$vname].'#';
		        		};break;
		        		case 'get':{ 
		        			if (isset($_GET[$vname]) && !empty($_GET[$vname])) 
		        			$dep_string.=$vname.':'.$_GET[$vname].'#';
		        		};break;
		        		case 'post':{ 
		        			if (isset($_POST[$vname]) && !empty($_POST[$vname])) 
		        			$dep_string.=$vname.':'.$_POST[$vname].'#';
		        		};break;
		        		case 'request':{ 
		        			if (isset($_REQUEST[$vname]) && !empty($_REQUEST[$vname])) 
		        			$dep_string.=$vname.':'.$_REQUEST[$vname].'#';
		        		};break;
			    	}				    	    		    	 
			    }
		    }
		    ...
		    switch ($cache_scale) {
		        ...
		        case 'global':{
		            $cache_id = 'global_'.$snippet_id;
		        };break;
		    }  
		    
		    $cache_id.= $dep_string;
			//echo $cache_id;
		    $this->cache_lite->interval += 60*1;
		    $lifetime = 60*10 + $this->cache_lite->interval;
		    
		    $options = array(
		        'cacheDir' => $modx->config['base_path'].'assets/cache/snippets/',
		        'lifeTime' => $lifetime,
		        'hashedDirectoryLevel' => 3,
		        'pearErrorMode' => CACHE_LITE_ERROR_DIE
		    );		    
		   
		    $this->cache_lite->init($options);
		    
		    if ($output = $this->cache_lite->get($cache_id,$cache_group)) {                
		        return $output;
		        //return 'C:-'.$cache_group;
        ...
        //if ($cache && !empty($snip) && !isset($php_errormsg)) $this->cache_lite->save($snip);
        $this->phx->curPass = 0;
        $snip = $this->phx->Parse($snip);
        if ($cache && !empty($snip)) $this->cache_lite->save($snip);
    ...
    function evalSnippets($documentSource) {
        ...
        //preg_match_all('~\[\!(.*?)\!\]~', $documentSource, $matches1);
    ...
    function getChildIdsOnly($id, $depth= 10, $children= array ()) {
        $c= null;
        foreach ($this->documentMap as $mapEntry) {
            if (isset ($mapEntry[$id])) {
                $childId= $mapEntry[$id];                
        ...
        if (is_array($c)) {
            ...
            if ($depth) {
                foreach ($c as $child) {
                    $children= $children + $this->getChildIdsOnly($child, $depth, $children);
                }
            }
        }
        return $children;
    ...
    function logEvent($evtid, $type, $msg, $source= 'Parser') {
        ...
        $ds= @mysql_query($sql);
        if (!$ds) {
            echo mysql_error() . "Error while inserting event log into database.";
            exit();
        }
    }
    ...
    function getWebUserInfo($uid) {
    	...
    	$sql= "
              SELECT wu.username, wu.password,wu.cachepwd, sr.name as region_name,se.name as state_name, se.postcode,wua.bonus,wg.webgroup as group_id,wua.*,wu.id as id
              FROM " . $this->getFullTableName("web_users") . " wu
              INNER JOIN " . $this->getFullTableName("web_groups") . " wg ON wg.webuser=wu.id
              INNER JOIN " . $this->getFullTableName("web_user_attributes") . " wua ON wua.internalkey=wu.id
              LEFT JOIN " . $this->getFullTableName("site_ec_regions") . " sr ON wua.region=sr.id
              LEFT JOIN " . $this->getFullTableName("site_ec_cities") . " se ON wua.state=se.id
              WHERE wu.id=$uid
              ";
        //
        //echo $sql;
