<?php if(IN_MANAGER_MODE!="true") die("<b>INCLUDE_ORDERING_ERROR</b><br /><br />Please use the MODx Content Manager instead of accessing this file directly."); 
	...
	$sql =	"SELECT DISTINCT parent FROM ".$modx->getFullTableName('site_ec_items');
	$rs = mysql_query($sql);
	$ec_item_pids = "";
	...
	if ($limit > 0) {
	    for ($i = 0; $i < $limit; $i++) {
	    	...
	    	$ec_item_pids .=$row['parent']. (($i != $limit-1) ? "," : "");	        
	    }	  
	}
...
<head>
    ...
    var ec_item_pids = [<?php echo $ec_item_pids;?>];
...
?>
    ...
    var itemselectorcontainer = null;
    var itemselectorloading = null;
    
    function rpcItemSelectorList(response) {
       if(itemselectorcontainer != null){
       if (typeof(response) =='object') itemselectorcontainer.innerHTML = response.responseText;
       else {
       	//alert(itemselectorlist.innerHTML);
       	itemselectorcontainer.innerHTML = response;
       }
       
       itemselectorcontainer.disabled = false;
       itemselectorcontainer.style.display = 'block';
       itemselectorcontainer.loaded = true;     
       itemselectorloading.innerHTML = '';      
   	   }
    }
   
    
    function loadItemSelectorList(id) {
        itemselectorcontainer = parent.main.$('itemselectorcontainer'); 
        itemselectorloading = parent.main.$('itemselectorloading');
        treeParams = 'a=900&ajaxcmd=getitemlist&parent='+id;
        itemselectorloading.innerHTML="<div class='loading'>Загрузка ...</div>";  
        new Ajax('index.php?'+treeParams, {method: 'get',onComplete:rpcItemSelectorList}).request();
    }
    ...
    function treeAction(id, name) {
    	...
    	if(ca == 'parentforitemselector') {
    		try {    			
                loadItemSelectorList(id);
                parent.main.$('similarParentName').innerHTML = name;
            } catch(oException) {
        ...
        if(ca=="open" || ca=="") {
            if(id==0) {
                ...
                if (ec_item_pids.indexOf(id) == -1) {
                	
                	parent.main.location.href="index.php?a=3&id=" + id + getFolderState(); //just added the getvar &opened=
                } else {
                	top.mainMenu.runtab("menu3-2_link");
                	parent.main.location.href="index.php?a=5000&pid=" + id + getFolderState(); //just added the getvar &opened=
...
function menuHandler(action) {
    switch (action) {
        ...
        case 5001 :
        	top.mainMenu.runtab("menu3-2_link");
            top.main.document.location.href="index.php?a=5001&pid=" + itemToChange;
            break    
        
        case 5000 :
        	top.mainMenu.runtab("menu3-2_link");
            top.main.document.location.href="index.php?a=5000&pid=" + itemToChange;
            break     
...
<div id="mx_contextmenu" onselectstart="return false;">
    ...
    <?php
    constructLink(5001, "newdoc", $_lang["ec_new_item"], $modx->hasPermission('ec_new_item'),$theme);   
    constructLink(5000, "context_view", $_lang["ec_list_items"], $modx->hasPermission('ec_list_items'),$theme);
    ?>
    <div class="seperator"></div>
    <?php
