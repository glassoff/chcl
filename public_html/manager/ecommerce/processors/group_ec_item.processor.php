...
include_once MODX_MANAGER_PATH . "ecommerce/actions/list_ec.items.dynamic.php";
...
if (isset($_POST['sel_items']) && sizeof($_POST['sel_items'])>0) {
	...
	##
	if(isset($_POST['allpages']) && $_POST['allpages']){
		$sel_ids = array();
	...
	$sel_ids = implode(',', $sel_ids);//die($sel_ids);
...
$header="Location: index.php?r=1&a=5000";	
		...
		break;	
			...
			$deltime = time();
			$sql = "UPDATE $dbase.`".$table_prefix."site_ec_items` SET action=1, editedby=".$modx->getLoginUserID().", editedon=".time()." WHERE id IN ($sel_ids) AND action=0;";
			$rs = mysql_query($sql);
			if(!$rs) {
				echo "Something went wrong while trying to edit the document...";
				exit;
		...
		break;	
			...
			$deltime = time();
			$sql = "UPDATE $dbase.`".$table_prefix."site_ec_items` SET action=0, editedby=".$modx->getLoginUserID().", editedon=".time()." WHERE id IN ($sel_ids) AND action=1;";
			$rs = mysql_query($sql);
			if(!$rs) {
				echo "Something went wrong while trying to edit the document...";
				exit;
