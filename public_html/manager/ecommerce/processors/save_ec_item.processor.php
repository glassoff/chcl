...
if($_POST['date_text']!='')
$date_text = mysql_escape_string($_POST['date_text']);
else $date_text='';
...
$producer = mysql_escape_string($_POST['producer']);

$composition = mysql_escape_string($_POST['composition']);
$material = mysql_escape_string($_POST['material']);
$color = mysql_escape_string($_POST['color']);

$size = mysql_escape_string($_POST['size']);
//$growth = mysql_escape_string($_POST['growth']);
...
$onec_code = mysql_escape_string((trim($_POST['1c_code'])));
...
$recommenditems = mysql_escape_string($_POST['recommenditems']);
...
if ($limit > 0) {
	for ($i = 0; $i < $limit; $i++) {
		...
		if ($row['type'] == 'uploadimage') {			 
			...
			$max_size = 1024*1024*30; // the max. size for uploading		
...
switch ($actionToTake) {
	case 'new' :
		...
		*/
		...
		$sql = "INSERT INTO $dbase.`" . $table_prefix . "site_ec_items` (recommenditems, similaritems,folder_code,date_issue,medium,dealer_cnt,mdealer_cnt,cds,parent,brand_id,pack_id,acc_id,1c_code,mdealer_price,dealer_price,retail_price,sku,menuindex,pagetitle, longtitle, published, template, new, popular,recommended, byorder, action, sell, soon, createdby, createdon, editedby, editedon,publishedby, publishedon,date_text, producer, vendor, country, size, composition, material, color, price_opt, package_price, package_items)
		VALUES('$recommenditems', '$similaritems','$folder_code','$date_issue','$medium','$dealer_cnt','$mdealer_cnt','$cds','$parent','$brand_id','$pack_id','$acc_id','".$onec_code."','$mdealer_price','$dealer_price','$retail_price','$sku','$menuindex','$pagetitle','$longtitle','$published','$template','$new','$popular','$recommended','$byorder', '$action',  '$sell','$soon',"."'$modx->getLoginUserID()', '" . time() . "', '" . $modx->getLoginUserID() . "', '" . time() . "', '" . $publishedby . "', '" . $publishedon ."', '" . $date_text ."' , 
																																																																																																									  
	'$producer', '$vendor', '$country', '$size', '$composition', '$material', '$color', '$price_opt', '$package_price', '$package_items'																																																																																																								  )";
		$rs = mysql_query($sql);
		if (!$rs) {
	...
	case 'edit' :
		...
		*/
		// update the document
		$sql = "UPDATE $dbase.`" . $table_prefix . "site_ec_items` SET recommenditems = '$recommenditems', similaritems = '$similaritems',folder_code = '$folder_code', date_issue='$date_issue', medium='$medium',dealer_cnt='$dealer_cnt',mdealer_cnt='$mdealer_cnt', introtext='$introtext', pagetitle='$pagetitle', longtitle='$longtitle', 
				cds = '$cds',parent='$parent', brand_id='$brand_id',pack_id='$pack_id', published='$published',template='$template', 
				acc_id='$acc_id',1c_code='".$onec_code."',sku=$sku,mdealer_price='$mdealer_price',dealer_price='$dealer_price',retail_price='$retail_price',
				popular='$popular', recommended='$recommended', byorder='$byorder', action='$action', sell='$sell', menuindex='$menuindex', new='$new', soon='$soon', editedby=" . $modx->getLoginUserID() . ", editedon=" . time() . ", publishedon=$publishedon, 
				publishedby=$publishedby, date_text='$date_text',
				
				producer='$producer', vendor='$vendor', country='$country', size='$size', composition='$composition', material='$material', color='$color', price_opt='$price_opt', package_price='$package_price', package_items='$package_items'	
				WHERE id=$id;";
		...
		$sql2 = "select * from $dbase.`".$table_prefix."notice`  WHERE disc_id=$id and active=1";
	...
	$row_rs2 = mysql_fetch_assoc($rs2);			
		...
		$sizes = $_POST['sizes'];
		$opt_prices = $_POST['opt_prices'];
		$retail_prices = $_POST['retail_prices'];
		$package_prices = $_POST['package_prices'];
		
		$sql = "DELETE FROM ".$modx->getFullTableName('site_ec_prices')." WHERE item_id='$id'";
		mysql_query($sql);
		...
		foreach($sizes as $i => $size_item){
			if (floatval($opt_prices[$i]) != $price_opt){
				...
				setPrice($id, $price_type, $size_item, $price_value);
			}
