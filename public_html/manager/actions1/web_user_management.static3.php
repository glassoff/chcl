...
if (isset($_SESSION['ec_ul_search'])) {
	...
	$sqladd = $search_user_id!="" ? " wua.internalKey=$search_user_id " : "" ;
	$sqladd .= $search_user_fname!="" ? "  wua.fname LIKE '%$search_user_fname%' " : "" ;
	$sqladd .= $search_user_sname!="" ? "  wua.sname LIKE '%$search_user_sname%' " : "" ;
	$sqladd .= $search_user_lname!="" ? "  wua.lname LIKE '%$search_user_lname%' " : "" ;
	$sqladd .= $search_user_email!="" ? "  wua.email LIKE '%$search_user_email%' " : "" ;
...
$grd->colTypes ="template:[+user_id+]";	
$grd->colTypes.="||template:<a href='index.php?a=88&id=[+user_id+]'>[+fname+] [+sname+] [+lname+]</a>";
$grd->colTypes.="||template:[+email+]";
$grd->colTypes.="||template:[+bonus+]%";
$grd->colTypes.="||template:<a href=\"index.php?a=5500&user_id=[+user_id+]\">$_lang[user_orders]</a>";
$grd->colTypes.="||template:[+blocked+]";
	
$grd->colTypes.=($modx->hasPermission('edit_web_user') ? '||template:<a href="#" title="'.$_lang['ec_edit_cust'].'" onclick="ec_edit_cust([+user_id+])"><img src="media/style/'.$manager_theme.'/images/icons/save.gif"></a>' : '');
$grd->colTypes.=($modx->hasPermission('delete_web_user') ? '<a href="#" title="'.$_lang['ec_delete_cust'].'" onclick="ec_delete_cust([+user_id+])"><img src="media/style/'.$manager_theme.'/images/icons/delete.gif"></a>' : '');
