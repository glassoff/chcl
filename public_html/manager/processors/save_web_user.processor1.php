...
switch ($_POST['mode']) {
	case '87' : // new user
		...
		$sql = "INSERT INTO $dbase.`" . $table_prefix . "web_user_attributes` (internalKey,fname,sname,lname,role, email, phone, comment, blocked, blockeduntil, blockedafter)
					VALUES($key,'$fname','$sname', '$lname', '$region', $state, '$roleid', '$email', '$phone', ''$comment', '$blocked', '$blockeduntil', '$blockedafter');";
		$rs = mysql_query($sql);
		if (!$rs) {
