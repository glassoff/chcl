...
if(!isset($_SESSION['mgrValidated'])) {
    ...
    $act_ses_timeout = time() + (60*$active_ses_time);
	$ip = $modx->getIP();		
	$currentsessionid = session_id();
	
	//$sql ="SELECT * FROM ".$modx->getFullTableName('active_managers'). " WHERE id='$internalKey' AND (ip <> '$ip' AND ses_id == '$currentsessionid')";
	$sql = "SELECT * FROM ".$modx->getFullTableName('active_managers'). " WHERE id='$internalKey' AND ip <> '$ip'";
	$rs = mysql_query($sql);
	$limit = mysql_num_rows($rs);	
	//jsAlert($limit);
	//return;
	if ($limit == 0) {
	   $sql1 ="SELECT * FROM ".$modx->getFullTableName('active_managers'). " WHERE id='$internalKey' AND ip = '$ip'";
	   $rs1 = mysql_query($sql1);
	   $limit1 = mysql_num_rows($rs1);	 	  
   	   if ($limit1 == 1) {   	       
		   $sql2 = "UPDATE ".$modx->getFullTableName('active_managers'). " SET act_ses_timeout = '$act_ses_timeout' WHERE id='$internalKey' AND ip = '$ip'"; 	       $do = mysql_query($sql2);
		   //echo mysql_error();			
   	   } else {   	   	  
		   $sql2 = "INSERT INTO ".$modx->getFullTableName('active_managers'). "(id,ip,act_ses_timeout,ses_id) VALUES('$internalKey', '$ip', '$act_ses_timeout', '$currentsessionid')";
		   $do = mysql_query($sql2);
		   //echo mysql_error();			   	  	
   	   }   	  
	} else {
		jsAlert("ѕользователь по имени $username уже авторизован!");
        return;		
	}
