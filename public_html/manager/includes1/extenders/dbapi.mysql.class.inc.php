...
class DBAPI {
   ...
   function connect($host = '', $dbase = '', $uid = '', $pwd = '', $persist = 0) {
      ...
      if (!$this->conn = ($persist ? mysql_pconnect($host, $uid, $pwd) : mysql_connect($host, $uid, $pwd, true))) {
         ...
         @mysql_query("SET CHARACTER SET 'cp1251_general_ci'",$this->conn);    
         mysql_query("SET NAMES 'cp1251';", $this->conn);
         mysql_query("SET character_set_results = cp1251';", $this->conn);
         mysql_query("SET collation_connection = 'cp1251_general_ci';", $this->conn);    
   ...
   function update($fields, $table, $where = "") {
      if (!$table)
         ...
         $where = ($where != "") ? "WHERE $where" : "";
         $this->sql = "UPDATE $table SET $flds $where";
