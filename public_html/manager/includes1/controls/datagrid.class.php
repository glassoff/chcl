...
class DataGrid {
	...
	var $showRecordInfo = true;
	var $useJSFilter = false;
	var $rowEventNames = '';
	var $rowEventFuncs = '';
	var $fields;
	var $columns;
	...
	function DataGrid($id,$ds,$pageSize=20,$pageNumber=-1) {		
		...
		// set datasource
		$this->ds = $ds; 
		$this->dsCount = mysql_num_rows($this->ds);
		// set pager
	...
	function RenderRowFnc($n,$row){		
		...
		$code = '';
		$func_code = '';
		$row_e_n = explode("||",$this->rowEventNames); 
		$row_e_h = explode("||",$this->rowEventFuncs); 
		
		if ((is_array($row_e_n) && is_array($row_e_h)) && (count($row_e_n) == count($row_e_h))) {
			
			foreach($row_e_n as $k1 => $eventname) {	
				$code = $row_e_h[$k1];			
				foreach($row as $k=>$v){	
					$v = htmlspecialchars(stripslashes($v));				
					$code = str_replace("[+$k+]",$v,$code);
				}				
				$func_code .= $eventname.'="'.$code.'" '; 
							
			}	
			
		}		
		$row_num = ($n) + ($this->pageNumber-1)*$pageSize;
		$o = "<tr  ".$func_code.">";
		for($c=0;$c<$this->_colcount;$c++){
			...
			$value = $this->formatColumnValue($row,$value,$type,$align,$row_num);
	...
	function formatColumnValue($row,$value,$type,&$align,$row_num = ''){
		global $modx, $_lang;
		...
		switch (strtolower($type)) {
			...
			case "template":
				...
				if(strpos($value,"[+")!==false) foreach($row as $k=>$v){	
					$v = htmlspecialchars(stripslashes(stripslashes($v)));								
					$value = str_replace("[+$k+]",$v,$value);
				}
				if($this->useJSFilter) 
					foreach($row as $k=>$v){										
						$value = str_replace("[+js.$k+]",mysql_escape_string($v),$value);
					}				
				break;
	...
	function render(){
		...
		// start grid
		$tblStart	= "<table width=\"100%\" $cssClass $cssStyle cellpadding='".(isset($this->cellPadding) ? (int)$this->cellPadding :1)."' cellspacing='".(isset($this->cellSpacing) ? (int)$this->cellSpacing:1)."'>";
		$tblEnd		= "</table>";
		...
		$from = ($this->pager->pageNumber-1)*$this->pager->pageSize + 1;
		$to = $from + $this->pager->cur_rows_count-1;
		...
		if(($tblPager && $ptop) || $this->showRecordInfo == true) 
		$o.="<tr><td align='".(substr($this->pagerLocation,-4)=="left"? "left":"right")."' $pagerClass $pagerStyle colspan='".($this->_colcount)."'>";
		$o.="<table width='100%' cellspacing='0' cellpadding='0'><td align='left' valign='top'>";		
		$o.="$_lang[ec_item_pages_view]&nbsp;$from-$to&nbsp;$_lang[ec_item_pages_view_from]&nbsp;$rowcount</td>";
		$o.="<td align='right' valign='top'>".$tblPager."&nbsp;</td></></table></td></tr>";
		$o.=$tblColHdr.$tblRows;
