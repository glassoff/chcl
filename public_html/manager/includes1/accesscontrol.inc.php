...
if (isset($_SESSION['mgrValidated'])) {	
	...
	//$sql =" SELECT * FROM ".$modx->getFullTableName('active_managers'). " WHERE id='$userid' AND ip = '$ip' AND ses_id = '$currentsessionid'";	
	$sql =" SELECT * FROM ".$modx->getFullTableName('active_managers'). " WHERE id='$userid' AND ip = '$ip'";	
	...
	if ($limit == 0 ) {				
		...
		$sql =" UPDATE ".$modx->getFullTableName('active_managers'). " SET act_ses_timeout = '$act_ses_timeout' WHERE id='$userid' AND ip = '$ip'";
		mysql_query($sql);
	}
