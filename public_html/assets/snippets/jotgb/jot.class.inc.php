...
class CJot {
	...
	function CJot() {
		...
		if (!class_exists('CChunkie')) include_once($path . '/includes/chunkie.class.inc.php');
	...
	function Run() {
		...
		$this->config["validate"] = !is_null($this->Get("validate")) ? $this->Get("validate") : "content:Вы оставили не незаполненными обязательные поля формы.";
		...
		$this->config["doc.pagetitle"] = !is_null($this->Get("pagetitle")) ? $this->Get("pagetitle"): '';
		...
		switch ($this->config["mode"]["passive"]) {
		  case "count-comments" :{ 
			  ...
			  echo  $output;
			  exit;
			  $modx->setPlaceholder('jot.comments.count',$output);
		  };break;
		...
		$modx->setPlaceholder('jot.comments.count',$comments_count);
	...
	function processForm($id=0) {
			...
			foreach($_POST as $n=>$v) {
						...
						switch($n) {
							...
							case 'content': // Content field
								...
								$pObj->Set("content",$v); 
								break;
							case 'reply': // Content field								
								$this->form["field"]["reply"] = $v;
								$pObj->Set("reply",$v); 								
								break;	
		...
		// Notify Subscribers
		if ($saveComment && $this->form['published']>0) {			
			$this->doNotifySubscribers($pObj->Get("id"));
		}
		
		$notify = isset($_REQUEST['notify']) ? intval($_REQUEST['notify']) : 0;
		if ($saveComment && $notify == 1) {			
			$this->doNotifyUser($pObj->Get("id"));
		}
		...
		// Notify Moderators
		if (!isset($_REQUEST['notify']) && $saveComment && (($this->form['published']==0) || ($this->form['published'] >0 && $this->config["moderation"]["notify"]==2))) $this->doNotifyModerators($pObj->Get("id"));
	...
	// Notifications
	function doNotifyUser($commentid=0) {
		global $modx;
		// Get comment fields
		$cObj = $this->provider;
		$cObj->Comment($commentid);
		...
		unset($cObj);
			...
			$user['email'] = $comment['fields']['email'];
			$user['name'] = $comment['fields']['name'];			
		...
		$tpl->AddVar('recipient',$user);
		$message = $tpl->Render();
		mail($email, $this->config["subject"]["subscribe"], $message, "From: ".$modx->config['site_name']."<".$modx->config['emailsender'].">");
		...
		if ($this->config["moderation"]["notify"]) {
		  ...
		  // Get comment fields
			...
			foreach ($moderators as $moderator){
				...
				$tpl->AddVar('recipient',$user);
				$message = $tpl->Render();
				mail($user["email"], $this->config["subject"]["moderate"], $message, "From: ".$modx->config['site_name']." <".$modx->config['emailsender'].">");
			}
		}
	}
	...
	function validateFormField($name = '', $value = '') {
		...
		foreach($validations as $validation) {
				switch ($validation["validation"]) {
					...
					case "email": {
						if (!empty($value))
						$re = "~^(?:[a-z0-9_-]+?\.)*?[a-z0-9_-]+?@(?:[a-z0-9_-]+?\.)*?[a-z0-9_-]+?\.[a-z0-9]{2,5}$~i"; else $re = '';
						break;
					}
				...
				if (!empty($re) && !preg_match($re,$value))
