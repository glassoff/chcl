...
class Wayfinder {
	...
	var $placeHolders = array(
		'rowLevel' => array('[+wf.wrapper+]','[+wf.classes+]','[+wf.classnames+]','[+wf.link+]','[+wf.title+]','[+wf.linktext+]','[+wf.id+]','[+wf.attributes+]','[+wf.docid+]','[+wf.introtext+]','[+wf.description+]','[+wf.subitemcount+]','[+wf.alias+]','[+wf.parent+]'),
	...
	function run() {
		...
		if ($this->_config['hereid'] != 0) $id = $this->_config['hereid']; else $id = $modx->documentIdentifier;
		$this->parentTree = $modx->getParentIds($id);
		$this->parentTree[] = $id;
	...
	function buildMenu() {
		global $modx;
		$outputTpl = $this->_templates['outerTpl'];
			...
			foreach ($subDocs as $parentId => $docs) {
				...
				if (!$this->_config['hideSubMenus'] || $this->isHere($parentId) || $level <= 1) {
					...
					if ($this->_config['columns'] > 1) {			
						$menuPart = $this->buildSubColMenu($parentId,$docs,$level);
						
						$output = str_replace("[+wf.wrapper+]",$menuPart,$outputTpl);
					} else {						
						$menuPart = $this->buildSubMenu($parentId,$docs,$level);
	...
	function buildSubColMenu($parentId,$menuDocs,$level) {
		global $modx;
		$subMenuOutput = '';
		$subMenuRowOutput = '';
		$firstItem = 1;
		...
		$cellOuter = $this->_templates['cellOuterTpl'];	
		
		$total = count($menuDocs);
		$columns = $this->_config['columns'];
		
		$rowspercol = floor($total/$columns);
		$mod = $total - $columns*$rowspercol;  
		...
		foreach ($menuDocs as $docId => $docInfo) {
			...
			if ($counter == $rowspercol+$dif) {
				$subMenuRowOutput .= str_replace('[+wf.wrapper+]', $subMenuOutput, $cellOuter);
				$counter = 0;
				$subMenuOutput = '';
				$mod--;
				if ($mod == 0) $dif = 0; 
		...
		if  (!empty($subMenuOutput)) $subMenuRowOutput .= str_replace('[+wf.wrapper+]', $subMenuOutput, $cellOuter);		
		//Return the submenu			
		return $subMenuRowOutput;
	}

	function buildSubMenu($parentId,$menuDocs,$level) {
		global $modx;
		$subMenuOutput = '';
		$firstItem = 1;
		$counter = 1;
		...
		if ($level > 0) {
			...
			$subMenuOutput = str_replace("[+wf.parent+]", $parentId, $subMenuOutput);
			$subMenuOutput = str_replace("[+wf.hereid+]", $this->_config['hereid'], $subMenuOutput);	
	 ...
	 function ultimateParent($id, $topLevel) {
        global $modx;
    ...
    function renderRow(&$resource,$numChildren) {
        ...
        $phArray = array($useSub,$useClass,$classNames,$resource['link'],$resource['title'],$resource['linktext'],$useId,$resource['link_attributes'],$resource['id'],$resource['introtext'],$resource['description'],$numChildren,$resource['alias'],$resource['parent']);
        ...
        if ($resource['last']) $output = str_replace('[+wf.lastclass+]',$this->_css['last'],$output); 
        if ($resource['here']) $output = str_replace('[+wf.hereclass+]',$this->_css['here'],$output);        
		//Return the row
		
		if (isset($this->_config['hereid']) && !empty($this->_config['hereid'])) {
			$act_id = $this->_config['hereid'];	
		} else {
			$act_id = $modx->documentIdentifier;
		}					
		
		$output = str_replace("[+wf.currparentlevel2+]", $this->ultimateParent($act_id, 2), $output);
		$output = str_replace("[+wf.currparentlevel3+]", $this->ultimateParent($act_id, 3), $output);		
    ...
    function setItemClass($classType, $docId = 0, $first = 0, $last = 0, $level = 0, $isFolder = 0, $type = 'document') {
        ...
        if ($classType === 'outercls' && !empty($this->_css['outer'])) {
            ...
            //if ($this->_config['hereid'] != 0) $this->_config['hereid'];
            //Set here class if specified
            if (!empty($this->_css['here']) && ($this->isHere($docId) || $this->_config['hereid'] == $docId)) {
