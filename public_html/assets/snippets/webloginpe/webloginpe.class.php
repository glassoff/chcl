 ...
 * @version 1.3.0
 * @access public
 * @copyright ©2007 Scotty Delicious http://scottydelicious.com
 **/
class WebLoginPE
{
	/**
	 ...
	 * @author Scotty Delicious
	 */
	function setCurentUser()
	{
		global $modx;
		if (!$modx->getLoginUserID()) return;
		$this->User = $modx->getWebUserInfo($modx->getLoginUserID());	
	...
	function getRegionsList()
	{
		...
		$lines = array();
		$lines[] = '<select class="reg_field" name="region" onchange="loadCities(this.options[this.selectedIndex].value)">';	
		$lines[] = '<option value="">-</option>';
		if ($rs && mysql_num_rows($rs)>0) {
			while ($row = mysql_fetch_assoc($rs)) {	
				
				if (@$this->User['region'] == $row['id'] || (isset($_POST['region']) && $_POST['region'] == $row['id'])) $lines[] = '<option value="'.$row['id'].'"  selected>'.$row['name'].'</option>';
	...
	function getStatesList()
	{
		...
		if (isset($this->User['region']) || isset($_POST['region'])) 
		{
		if (isset($_POST['region'])) $region = intval($_POST['region']);
		else $region = $this->User['region'];	
		...
		$lines[] = '<select class="reg_field" name="state">';				
			if ($rs && mysql_num_rows($rs)>0) {
				while ($row = mysql_fetch_assoc($rs)) {			
					if (@$this->User['state'] == $row['id'] || (isset($_POST['state']) && $_POST['state'] == $row['id'])) $lines[] = '<option value="'.$row['id'].'"  selected>'.$row['name'].'</option>';
					else $lines[] = '<option value="'.$row['id'].'"  >'.$row['name'].'</option>';
				}		
			} else {
	...
	function Login($type, $liHomeId = '')
	{
		...
		$this->Username = $modx->db->escape(strip_tags($_REQUEST['username']));
		$this->Password = $modx->db->escape(strip_tags($_REQUEST['password']));
	...
	function Login_($username,$password)
	{
		...
		$this->ActiveUsers();
		$this->UserDocumentGroups();			
		if ($modx->getLoginUserID('web')) return true; 
		else return false;
	}
		...
		if ($tableCheck == 1)
		{
			...
			while ($eachTable = $modx->db->getRow($tableNames, 'num'))
	 		...
	 		if (!in_array($table, $allTables))
	 		{
				...
				if (!$createTable)
				{
					$this->FormatMessage($modx->db->getLastError);
					return;
				}
	...
	##
	function saveClientFile($user_id){
		$files_path = "/assets/files/clientfiles/";

		if (is_uploaded_file($_FILES['clientfile']['tmp_name'])) {
			$filename = $_FILES['clientfile']['tmp_name'];
			
			$file_dir = $files_path.$user_id;
			if(!file_exists(MODX_BASE_PATH.$file_dir)){ 
				if( !@mkdir(MODX_BASE_PATH.$file_dir) )
					return false;
			}
			//@chmod(MODX_BASE_PATH.$file_dir, 0777);
				
			$new_filename = $_FILES['clientfile']['name'];
			$new_filename = $this->encodestring($new_filename);
					
			if (@move_uploaded_file($filename, MODX_BASE_PATH . $file_dir . "/" . $new_filename)) {
...

  {
	...
	function Register($regType, $groups, $regRequired, $notify, $notifyTpl, $notifySubject)
	{
		...
		$this->Username = $username;
	    $password = $modx->db->escape($modx->stripTags($_REQUEST['password']));
		$passwordConfirm = $modx->db->escape($modx->stripTags($_POST['passwordconfirm']));
	    $fname = $modx->db->escape($modx->stripTags($_POST['fname']));
	    $sname = $modx->db->escape($modx->stripTags($_POST['sname']));
	    $lname = $modx->db->escape($modx->stripTags($_POST['lname']));
	    ...
	    $phone = $modx->db->escape($modx->stripTags($_POST['phone']));
		    ...
		    $type = $modx->db->escape($modx->stripTags($_POST['type']));
		    $company= $modx->db->escape($modx->stripTags($_POST['company']));
		    $opt = $modx->db->escape($modx->stripTags($_POST['opt']));
		
		
	
	$region = $modx->db->escape($modx->stripTags($_POST['region']));
		$town = $modx->db->escape($modx->stripTags($_POST['town']));
		$street = $modx->db->escape($modx->stripTags($_POST['street']));
		$house = $modx->db->escape($modx->stripTags($_POST['house']));
		$kvartira = $modx->db->escape($modx->stripTags($_POST['kvartira']));
		$korpus = $modx->db->escape($modx->stripTags($_POST['korpus']));
		$postcode1 = $modx->db->escape($modx->stripTags($_POST['postcode1']));
		
		$bonuscode = $modx->db->escape($modx->stripTags($_POST['bonuscode']));
		$infosource = $modx->db->escape($modx->stripTags($_POST['infosource']));
		$infosource_other = $modx->db->escape($modx->stripTags($_POST['infosource_other']));
		...
		$subscribe = $modx->db->escape($modx->stripTags($_POST['subscribe']));
		...
		//exclude required
		if($type=='Физ.лицо'){
			$regRequired = str_replace('company', '', $regRequired);
		}
		...
		if ($regRequired !== '')
		{
			...
			for($i = 0; $i < count($requiredFields); $i++){
				if(trim($requiredFields[$i])=='') unset($requiredFields[$i]);
			}
			$empty_err = false;
			...
			foreach ($requiredFields as $field)
			{
				if (empty($_POST[$field])) {
					$modx->setPlaceholder($field.'.empty','1');
			...
			foreach ($requiredFields as $field)
			{
				...
				if ($field == 'email')
				{
					...
					$email_check =  $this->ValidateEmail($_POST['email']);
					if (!$email_check)
					{
						$this->invalidFields[] = 'email';
						$this->FormatMessage($this->LanguageArray[45]);						
						return; 
					}
				}			
			}
		...
		if (strlen($badUsername) !== strlen($username))
		{
			$this->FormatMessage($this->LanguageArray[32]);
			return;
		}
		...
		{
			$this->FormatMessage($this->LanguageArray[1]);
			return;
		}
		if (!empty($postcode1))
		if ( (strlen($postcode1) != 6  || (intval($postcode1) == 0) || (strval(intval($postcode1)) != $postcode1)) )
			{
				$this->FormatMessage($this->LanguageArray[106]);
				return; 
			}
			
		if (!empty($bonuscode))
		if ( !preg_match('/^[a-zA-Z0-9]{2,4}$/', $bonuscode) )
			{
				$this->invalidFields[] = 'bonuscode';
				$this->FormatMessage($this->LanguageArray[108]);
				return; 
			}			
		...
		if ($regType == 'instant')
		{
			...
			if (md5($password) !== md5($_REQUEST['password']))
			{
				$this->invalidFields[] = 'password';
		...
		{
			$this->invalidFields[] = 'email';
			$this->FormatMessage($this->LanguageArray[8]);
			return; 
		}
		...
		if (!$createNewUser)
		{
			$this->FormatMessage($this->LanguageArray[9]);
			return; 
		} 
		...
		##
	    if($_FILES['clientfile']['name']){
	    	if ($clientfile = $this->saveClientFile($key)){
	    		
	    	}
	    	else{
				$this->invalidFields[] = 'clientfile';
				$this->FormatMessage("Ошибка загрузки карточки клиента");
				return;
	    	}
	    }		
		//die($clientfile);
		...
		" (internalKey,reg_date,fname,sname,lname, email, phone, region, town, street, house, korpus, kvartira, postcode1, type, company, opt, clientfile, bonuscode, infosource, subscribe) VALUES".
		" ('".$key."',".$reg_date.",'".$fname."', '".$sname."', '".$lname."', '".$email."', '".$phone."', '".$region."', '".$town."', '".$street."','".$house."','".$korpus."', '".$kvartira."','".$postcode1."','".$type."','".$company."','".$opt."', '".$clientfile."', '".$bonuscode."', '".$infosource."', '".$subscribe."')";
		$insertUserAttr = $modx->db->query($newUserAttr);
				
		if (!$insertUserAttr)
		{
			$this->FormatMessage($this->LanguageArray[10]);
			return; 
		}
		...
		if (count($groupArray > 0))
		{
			...
			if (!$groupNames)
			{
				$this->FormatMessage($this->LanguageArray[11]);
				return;
			}
			else
			{
		...
		// EVENT: OnWebSaveUser		
		$this->User = $modx->getWebUserInfo($key);	
		//$this->UserIsBlocked();
		$this->SessionHandler('start');			
		$this->OnWebLogin();
		$this->UserDocumentGroups();
		...
		$this->OnWebSaveUser('new', $NewUser);// Replace some placeholders in the Config websignupemail message.
		...
		if (!$Register->Send())
		{
			$this->FormatMessage($this->LanguageArray[12]);
			return; 
		}
		...
		$notification = str_replace('[+type+]', $type, $notification);
		$notification = str_replace('[+company+]', $company, $notification);
			...
			foreach ($emailArray as $address)
			{
				...
				if (!$Notify->Send())
				{
					$this->FormatMessage($Notify->ErrorInfo);
					return; 
				}
		...
		$this->FormatMessage($this->LanguageArray[100]);
		return 'success';
	}
	 ...
	 * @author Scotty Delicious
	 */
	function SaveUserProfile($internalKey = '',$requiredFields = '')
	{
		global $modx;
		...
		$oldusername = $currentWebUser['username'];
		
		$web_users = $modx->getFullTableName('web_users');
		$web_user_attributes = $modx->getFullTableName('web_user_attributes');
		
		if (isset($_REQUEST['changelogindatas']) && !empty($_REQUEST['changelogindatas'])) {
			$new_username = $modx->db->escape(trim($_POST['username']));
			
			if ($currentWebUser['username'] != $new_username) {			
				$checkUsername = $modx->db->query("SELECT `id` FROM ".$web_users." WHERE `username`='".$new_username."' AND id <> $internalKey ");
				$limit = $modx->recordCount($checkUsername);
				...
				$usernameElement = "UPDATE ".$web_users." SET `username`='".$new_username."' WHERE `id`='".$internalKey."'";
				$saveMyUsername = $modx->db->query($usernameElement);
				##
				$emailElement = "UPDATE ".$web_user_attributes." SET `email`='".$new_username."' WHERE `internalkey`='".$internalKey."'";
				$saveMyEmail = $modx->db->query($emailElement);
			...
			if (!empty($_REQUEST['newpassword']) && isset($_REQUEST['newpassword']) && isset($_POST['newpasswordconfirm']))
			{
				if ($_REQUEST['newpassword'] === $_POST['newpasswordconfirm'])
				{
					if (md5($_REQUEST['newpassword']) === md5($modx->db->escape(strip_tags($_REQUEST['newpassword']))))
					{
						if (strlen($_REQUEST['newpassword']) > 5)
						{
							$new_password = trim($_REQUEST['newpassword']);	
							
							$passwordElement = "UPDATE ".$web_users." SET `password`='".md5($modx->db->escape(trim($_REQUEST['newpassword'])))."' WHERE `id`='".$internalKey."'";
			...
			if ($password_changed) {			
				...
				$user = $modx->getWebUserInfo($internalKey);
				
				$this->OnWebChangePassword($internalKey, $user['username'], $user['password']);	
				$this->OnWebLogin();	
					
				if (isset($_COOKIE['WebLoginP']) && $_COOKIE['WebLoginP'] !== '')
				{
					$cookieName = 'WebLoginPE';
					$cookieValue = md5($user['username']).'|'.$user['password'];
					$cookieExpires = time() + 86400;					
					setcookie($cookieName, $cookieValue, $cookieExpires, '/', $_SERVER['SERVER_NAME'], 0);
				}	
				
				$_SESSION['web_password_changed'] = true;				
				$messageTpl = $modx->config['webpwdreminder_message'];					
				$myEmail = $modx->config['emailsender'];
				$emailSubject = $modx->config['webpwdreminder_subject'];
				$siteName = $modx->config['site_name'];
				$siteURL = $modx->config['site_url'];	
				$fullname = $user['fname'].' '.$user['sname'].' '.$user['lname'];
				$email = $user['email'];			
				$message = str_replace('[+uid+]', $user['username'], $messageTpl);
				$message = str_replace('[+pwd+]', $new_password, $message);
				$message = str_replace("[+ufn+]", $fullname, $message);
            	$message = str_replace('[+sname+]', $siteName, $message);
				$message = str_replace('[+semail+]', $myEmail, $message);
				$message = str_replace('[+surl+]', $siteURL, $message);
				...
				$Register->FromName = $siteName;
				$Register->Subject = $emailSubject;
				$message.= $modx->config['webemail_signature'];
				$Register->Body = $message;
				$Register->AddAddress($email, $fullname);
			...
			if ($username_changed) {
				$_SESSION['web_username_changed'] = true;
				$this->User = $user = $modx->getWebUserInfo($internalKey);
				$this->User['oldusername'] = $oldusername;
		...
		if ($requiredFields !== '')
		{
			$requiredFields = explode(',', str_replace(' ,', ',', $requiredFields));
			foreach ($requiredFields as $field)
			{
				if (empty($_POST[$field])) {
					$this->FormatMessage($this->LanguageArray[0]);
		...
		foreach ($generalElementsArray as $id => $field)
		{
			...
			if (isset($_POST[$field]))
			{
				...
				$generalElementsUpdate[] = " `".$field."` = '".$modx->db->escape(stripslashes(trim($_POST[$field])))."'";
			}
			else
			{
		...
		$generalElementsUpdate[] = $_POST['subscribe']=='1' ? " `subscribe` = '1'" : " `subscribe` = '0'"; 
		##
		if($_FILES['clientfile'] && $_FILES['clientfile']['error'] != 4){
			$sql = "SELECT clientfile FROM ".$web_user_attributes." WHERE `internalkey` = '".$internalKey."'";
			$checkclientfile = $modx->db->query($sql);
			$check_row = $modx->db->getRow($checkclientfile);
			$oldclientfile = $check_row['clientfile'];
			
	    	if ($clientfile = $this->saveClientFile($internalKey)){
	    		if($oldclientfile)
	    			unlink(MODX_BASE_PATH . $oldclientfile);
	    		$generalElementsUpdate[] = " `clientfile` = '".$modx->db->escape(stripslashes(trim($clientfile)))."'";
	    	}
	    	else{
				$this->FormatMessage("Ошибка загрузки карточки клиента");
				return;
	...
	function RemoveProfile($internalKey)
	{
		...
		{
			$this->FormatMessage($this->LanguageArray[13]);return;
		}
	...
	function SendMessageToUser()
	{
		...
		$EmailMessage->CharSet="windows-1251";
	...
	function ResetPassword()
	{
        ...
        $webpwdreminder_message = $modx->config['webchangepassword_message'];
        $emailsubject = $modx->config['webchangepassword_emailsubject'];
		...
		if ($limit == 1 && !empty($email))
		{
			
			// Reset the password and fire off an email to the user
			//$newPassword = $this->GeneratePassword(10);
			$newPasswordKey = $this->GeneratePassword(32);
			$this->User = $modx->db->getRow($userInfo);
			$insertNewPassword = "UPDATE ".$web_users." SET cachepwd='".$newPasswordKey."' WHERE id='".$this->User['internalKey']."'";
            ...
            $message = str_replace("[+ufn+]", $this->User['fname'].' '.$this->User['sname'].' '.$this->User['lname'], $message);
	...
	function ActivateUser()
	{
		...
		$newPasswordConfirm = $modx->db->escape($_POST['newpasswordconfirm']);
		...
		if ($limit !==1)
		{
			$this->FormatMessage($this->LanguageArray[15]);
			return;
		}
		
		$this->User = $modx->getWebUserInfo($userid);
		$cacheKey = $this->User['cachepwd'];
		
		//if (($passwordKey !== $cachePassword) || ($activationKey !== $cacheKey))
		if ($activationKey !== $cacheKey)
		{
			
			$this->FormatMessage($this->LanguageArray[16]);
			return;
		}
		...
		{ 
			$this->FormatMessage($this->LanguageArray[17]);
			return;
		}
			...
			if (!empty($CurrentUser))
			{
				foreach ($CurrentUser as $key => $value)
				{
					...
					##
						...
						$modx->setPlaceholder('user.clientfileload', '<a style="margin-left: 6px;" href="'.MODX_SITE_URL.''.$value.'">'.basename($value).'</a>'); 
					}
					else if ($key == 'dob')
					{
	...
	function Authenticate()
	{
		...
		$cacheTimestamp = $modx->db->query("UPDATE ".$web_users." SET `cachepwd`='' WHERE `id`='".$this->User['id']."'");
 	}
	...
	function LoginHomePage()
	{
		...
		if (isset($_SESSION['AFTER_LOGIN_GO_URL'])) {
			$url = $_SESSION['AFTER_LOGIN_GO_URL'];
  		    unset($_SESSION['AFTER_LOGIN_GO_URL']);
  		    //echo $url;
  		    //exit; 
  		    //session_write_close();
			$modx->sendRedirect($url,0,'REDIRECT_HEADER');  		    
	...
	function SessionHandler($directive)
	{
		...
		if ($directive == 'start') 
		{
		    ...
		    $_SESSION['webFname'] = $this->User['fname']; 
		    $_SESSION['webSname'] = $this->User['sname'];
		    $_SESSION['webLname'] = $this->User['lname'];
		...
		if ($directive == 'destroy')
		{
	        ...
	        if (isset($_SESSION['mgrValidated']))
			{
	            ...
	            unset($_SESSION['webFname']);
	            unset($_SESSION['webSname']);
	            unset($_SESSION['webLname']);	                     
	...
	function QueryDbForUser($Username)
	{
		...
		$query = "SELECT * FROM ".$web_users.", ".$web_user_attributes.", ".$this->CustomTable." WHERE  ".$web_users.".username = '".$Username."' AND ".$web_user_attributes.".`internalKey` = ".$web_users.".`id` AND ".$this->CustomTable.".`internalKey` = ".$web_users.".`id`";
		$dataSource = $modx->db->query($query);
		$limit = $modx->db->getRecordCount($dataSource);
		
		if ($limit == 0)
		{
			$query = "SELECT * FROM ".$web_users.", ".$web_user_attributes." WHERE ".$web_users.".username = '".$Username."' AND ".$web_user_attributes.".`internalKey` = ".$web_users.".`id`";
			$dataSource = $modx->db->query($query);
			$limit = $modx->db->getRecordCount($dataSource);
		}
	...
	function UserIsBlocked()
	{
		...
		{ // this user has been blocked by an admin, so no way he's loggin in!
	        $this->SessionHandler('destroy');
	        $this->FormatMessage($this->LanguageArray[23]);
	        return;
	    }
		...
		if (isset($modx->config['allowed_ip']))
		{
			...
			{
	            $this->FormatMessage($this->LanguageArray[25]);
	            return;
	        }
	    }
		...
		if (isset($modx->config['allowed_days']))
		{
			...
			{
	            $this->FormatMessage($this->LanguageArray[26]);
	            return;
	        }        
	    }
	...
	function MakeDateForDb($date)
	{
		...
		{
			$this->FormatMessage($this->LanguageArray[27]);
			return;
		}
	...
	function CreateUserImage()
	{
		...
		{
			$this->FormatMessage($this->LanguageArray[29]);
			return;
		}
		...
		switch ($_FILES['photo']['type'])
		{
			...
			default	:
				$this->FormatMessage($this->LanguageArray[30]);
				return;
				break;
		}
	...
	function OnWebLogin()
	{
		...
		if(isset($_POST['rememberme'])) $parameters['rememberme'] = $_POST['rememberme'];	 	
	...
	function OnWebDeleteUser($internalKey, $username)
	{
		...
		$modx->invokeEvent('OnWebDeleteUser', $paramaters);
	}
	...
	function OnBeforeWebLogout()
	{
		...
		$modx->invokeEvent('OnBeforeWebLogout', $paramaters);
	}
	...
	function OnWebLogout()
	{
		...
		$modx->invokeEvent('OnWebLogout', $paramaters);
	}
