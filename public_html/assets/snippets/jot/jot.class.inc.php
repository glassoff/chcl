...
class CJot {
	...
	function Run() {
		...
		if($this->config["captcha"] == 2) { if ($this->config["user"]["id"]) {	$this->config["captcha"] = 1;} else { $this->config["captcha"] = 1;} }
	...
	function processForm($id=0) {
		...
		//-- Captcha/Veriword
		if ($saveComment && !(( isset($_POST['vericode']) && isset($_SESSION['veriword']) && $_SESSION['veriword'] == $_POST['vericode']))) {
