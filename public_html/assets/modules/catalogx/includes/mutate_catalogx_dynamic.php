...
if (!isset ($_REQUEST['cx_id'])) {
    $id = 0;
} else {
    $id = !empty ($_REQUEST['cx_id']) ? intval($_REQUEST['cx_id']) : 0;
}
...
if (!empty ($id)) {
    $tblsc = $dbase . ".`" . $table_prefix . "site_catalogx`";
...
if (!isset ($_REQUEST["cx_id"])) {
    if (!isset ($auto_menuindex) || $auto_menuindex) {
        ...
        $tbl = $modx->getFullTableName("site_catalogx");
...
function deletedocument() {
    if(confirm("<?php echo $_lang['confirm_delete_document']; ?>")==true) {
        document.location.href="index.php?id=" + document.mutate.id.value + "&cxa=6";
    }
}
...
function setParent(pId, pName) {
    if (!allowParentSelection) {
        window.location.href="index.php?cxa=3&id="+pId;
        return;
    }
    else {
...
function templateWarning(){
    ...
    if (confirm('<?php echo $_lang['tmplvar_change_template_msg']?>')){
        documentDirty=false;
        document.mutate.cxa.value = <?php echo $action; ?>;
...
if (is_array($evtOut))
    echo implode("", $evtOut);
?>
<input type="hidden" name="cxa" value="5" />
<input type="hidden" name="id" value="<?php echo $content['id'];?>" />
<input type="hidden" name="mode" value="<?php echo @$_REQUEST['cxa'];?>" />
<input type="hidden" name="MAX_FILE_SIZE" value="<?php echo isset($upload_maxsize)? $upload_maxsize:3048576; ?>" />
...
<div class="subTitle">
    ...
    <script type="text/javascript">
        <?php if($_REQUEST['cxa']=='4' || $_REQUEST['cxa']==72) { ?>document.getElementById("Button2").className='disabled';<?php } ?>
    </script>
    <div class="stay">
...
if ($content['type'] == "reference" || $_REQUEST['cxa'] == 72) {
...
?>
            ...
            <?php if($content['type']=="reference" || $_REQUEST['cxa']==72) { ?>
              <tr style="height: 24px;">
...
if (isset ($_REQUEST['cx_id'])) {
    if ($content['parent'] == 0) {
        ...
        $sql = "SELECT pagetitle FROM $dbase.`" . $table_prefix . "site_catalogx` WHERE $dbase.`" . $table_prefix . "site_catalogx`.id = " . $content['parent'] . ";";
                    $rs = mysql_query($sql);
                    $limit = mysql_num_rows($rs);
        if ($limit != 1) {
                        $e->setError(8);
    ...
    if (isset ($_REQUEST['pid'])) {
        if ($_REQUEST['pid'] == 0) {
            ...
            $sql = "SELECT pagetitle FROM $dbase.`" . $table_prefix . "site_catalogx` WHERE $dbase.`" . $table_prefix . "site_catalogx`.id = " . $_REQUEST['pid'] . ";";
                    $rs = mysql_query($sql);
                    $limit = mysql_num_rows($rs);
            if ($limit != 1) {
                        $e->setError(8);
...
if (isset ($content['id']) && count($keywords) > 0) {
    $keywords_selected = array ();
        $tbl = $modx->getFullTableName("catalogx_keyword_xref");
...
if (isset ($content['id']) && count($keywords) > 0) {
    $metatags_selected = array ();
        $tbl = $modx->getFullTableName("site_catalogx_metatags");
...
<?php } ?>
    ...
    <?php if($_REQUEST['cxa']!='4' && $_REQUEST['cxa']!=72) { ?>
        <!-- Preview -->
...
<!-- Content -->
<?php if($content['type']=="document" || $_REQUEST['cxa']==4) { ?>
<div class="sectionHeader"><?php echo $_lang['document_content']; ?></div><div class="sectionBody">
    <?php
    
if ((@$content['richtext'] == 1 || $_REQUEST['cxa'] == 4) && $use_editor == 1) {
        // replace image path
...
<!-- Template Variables -->
<?php if (($content['type'] == "document" || $_REQUEST['cxa'] == 4) || ($content['type']=="reference" || $_REQUEST['cxa']==72)) { ?>
...
<?php
    ...
    $sql .= "LEFT JOIN $dbase.`" . $table_prefix . "site_tmplvar_catalogxvalues` tvc ON tvc.tmplvarid=tv.id AND tvc.contentid = $id ";
...
if ($use_udperms == 1) {
    ...
    if ($_REQUEST['cxa'] == '27') { // fetch permissions on the document from the database
...
if ($content['type'] == "document" || $_REQUEST['cxa'] == 4) {
    if ((@$content['richtext'] == 1 || $_REQUEST['cxa'] == 4) && $use_editor == 1) {
        if (is_array($replace_richtexteditor)) {
