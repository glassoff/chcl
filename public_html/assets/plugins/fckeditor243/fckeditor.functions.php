...
{$toolbarOptions}
          ...
          <tr id='fck_customset1' class='row3' style="display: {$cusDisplay}"> 
            ...
            <td>
            <input name="fck_editor_toolbar_customset" type="text" style="width:300px;" maxlength='65000' onChange="documentDirty=true;" value="{$toolbarcustom}" />
			</td> 
          </tr> 
...
if (!function_exists('getFCKEditorScript')) {
	function getFCKEditorScript($elementList,$toolbarset,$toolbarcustom,$width,$height,$language,$frontend,$base_url,$editor_css_path,$use_browser,$autoLang,$editorstyle,$pluginfolder) {
		...
		foreach($elementList as $fckInstance) {
			...
			$fckInstances .= "$fckInstanceObj.BaseHref = '".$site_url."';";
			$fckInstances .= "$fckInstanceObj.BasePath = '".$base_url."assets/plugins/".$pluginfolder."/';";
			$fckInstances .= "$fckInstanceObj.Config['ImageUpload'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= "$fckInstanceObj.Config['ImageBrowser'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= ($use_browser==1  && $frontend=='false' ? "$fckInstanceObj.Config['ImageBrowserURL'] = FCKImageBrowserURL;" : "");
			$fckInstances .= "$fckInstanceObj.Config['LinkUpload'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= "$fckInstanceObj.Config['LinkBrowser'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= ($use_browser==1  && $frontend=='false' ? "$fckInstanceObj.Config['LinkBrowserURL'] = FCKLinkBrowserURL;" : "");
			$fckInstances .= "$fckInstanceObj.Config['FlashUpload'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= "$fckInstanceObj.Config['FlashBrowser'] = ".($use_browser==1  && $frontend=='false' ? "true":"false").";";
			$fckInstances .= ($use_browser==1  && $frontend=='false' ? "$fckInstanceObj.Config['FlashBrowserURL'] = FCKFlashBrowserURL;" : "");
		...
		$browserConfig .= ($use_browser==1 && $frontend=='false') ? "			var FCKImageBrowserURL = '{$base_url}manager/media/browser/mcpuk/browser.html?Type=images&Connector={$base_url}manager/media/browser/mcpuk/connectors/php/connector.php&ServerPath={$base_url}';\n" : "";
		$browserConfig .= ($use_browser==1 && $frontend=='false') ? "			var FCKLinkBrowserURL = '{$base_url}manager/media/browser/mcpuk/browser.html?Connector={$base_url}manager/media/browser/mcpuk/connectors/php/connector.php&ServerPath={$base_url}';\n" : "";
		$browserConfig .= ($use_browser==1 && $frontend=='false') ? "			var FCKFlashBrowserURL = '{$base_url}manager/media/browser/mcpuk/browser.html?Type=flash&Connector={$base_url}manager/media/browser/mcpuk/connectors/php/connector.php&ServerPath={$base_url}';\n" : "";
